/**
 * Traditional Approach (Without Redis):
 * > Socho hum MySQL use kr rhe hai.
 *   - Jab bhi humein data read/write (query/mutate) karna hota hai,
 *     hum direct MySQL DB ko hit krte hai
 *   - DB query run hoti hai
 *   - Result wapas milta hai
 * > Ab problem yahan aati hai
 *   - Same query baar-baar chal rhi hai
 *   - Data change nhi ho rha
 *   - Phir bhi har baar request pe DB hit ho rhi hai
 * > Iske liye:
 *   - High Latency (slow reponse)
 *   - Unnecessary DB load
 *   - Poor scalability
 * > Basically hum direct access kar rhe hai MySQL se every time.
 * 
 * Solution: Redis (Remote Dictionary Server)
 * > RAM is super fast compared to disk-based DBs:
 *   - Frequently used data ko
 *   - DB k bajaye
 *   - Memory (RAM) mein rakh do
*/


/**
 * REDIS:
 * > Redis is open-source, in-memory, NoSQL, Key-Value Store.
 * > Redis:
 *   1. Stores frequently accessed data
 *   2. Serves data without hitting DB
 *   3. Reduces DB load
 *   4. Improves response time massively
 * > Commong Use Cases:
 *   1. Caching            : API Responses, DB Query Results
 *   2. Session Storage    : Login Sessions, Auth tokens
 *   3. Real Time Analytics: Page views, Active Views, Online Users
 *   4. Messaging Systems  : Pub/Sub
 *   5. Background Jobs    : Queues, Task Processing
 *   6. Temporary Storage  : OTPs, Verification Tokens, Expiring data
 *   7. Rate Limiting      : Limit Login attempts, Prevent API abuse
 * 
 * Note: Redis is used when speed matters more than permanent storage.
*/

/**
 * Jargons:
 * 1. Redis      : CLI of Redis
 * 2. Redis Stack: GUI of Redis
 * 
 * Service like Redis:
 * 1. Redis Cloud
 * 2. AWS ElastiCache
 * 3. Azure Cache for Redis
 * 4. Upstash (we'll use this)
 * 
 * Why Upstash Redis?
 * > You don't want Docker today
 * > Upstash:
 *   - Serverless Redis
 *   - No Docker, No infra
 *   - Works perfectly with Node.js
 *   - Free tier for learning
 *   - HTTPS based (REST + Redis Protocol)
*/

/**
 * Upstash Redis Setup:
 * 1. Create Redis Database
 *    a. Go to upstash.com
 *    b. Create account
 *    c. Create Redis database
 *    d. Copy:
 *       - REDIS_URL
 *       - REDIS_TOKEN
 * 
 * 2. Connecting Redis in Node.js (No Docker)
 *    a. Install Client: npm install @upstash/redis
 *    b. Create redis client
 *       import { Redis } from "@upstash/redis";
 *       
 *       export const redis = new Redis({
 *         url: process.env.REDIS_URL,
 *         token: process.env.REDIS_TOKEN,
 *       });
 * 
*/

/**
 * Redis Docker Setup:
 * > docker run -d --name redis-stack -p 6379:6370 -p 8001:8001 redis/redis-stack:latest
 * > Copy and keep the Id given in the terminal
 * > Go to http://locahost:8001 (Redis Stack - GUI)
 * > Run: docker ps and copy containerId
 * > Run: docker exec -it redis-stack redis-cli
 *   - Input : PING
 *   - Output: PONG
*/


/**
 * Core Datatypes in Redis:
 * 1. String
 * 2. List
 * 3. Hash
 * 4. Set
 * 5. Zset (Sorted Set)
 * 6. Stream
 * 7. Bitmaps
 * 8. Hyperlog
 * 9. Geospatial
 * 10. Timeseries
 * ....
*/


/**
 * Redis String Data Type:
 * 1. Setting Key-Value Data 
 *    > Open Redis CLI in Terminal:
 * 
 *    > Approach-1: set user "John Doe"
 *    > Approach-2: set user:1 "Suraj"
 *    > Approach-3: set user:2:name "Suraj" (Recommended)
 * 
 *    Note: Colon : ka use logical grouping ke liye hota hai.
 * 
 * 2. Set Expiration time (TTL):
 *    Ye Session jaise data ke liye useful:
 * 
 *     a. Input : set session:abc123 "user_data" ex 10 
 *        Output: ex 10 - 10sec ke baad data auto-delete
 *
 *     b. Input : set session:abc123 "user_data" ex 10
 *        Output: Old value override ho jayegi
 * 
 * 3. Increment and Decrement Operations:
 *    Counters ke liye perfect
 *    - Input : set notification 90
 *    - Output: OK
 *    - Input : set notification 90
 *    - Output: OK
 *    - Input : incr notification
 *    - Output: (integer) 91
 *    - Input : get notification
 *    - Output: "91"
 *    - Input : decr nofitication
 *    - Output: (integer) 90
 *    - Input : set counter 10
 *    - Output: OK
 *    - Input : incrby counter 50
 *    - Output: (integer) 60
 *    - Input : incrbyfloat counter 5.0
 *    - Output: "65"
 *    - Input : incrbyfloat counter 14.5
 *    - Output: "79.5"
 * 
 * 4. Flags:
 *    Flags control how SET behaves.
 *    a. nx: Only set if key doesn't exist
 *    b. xx: Only set if this key exists
 *    c. ex: Set expiration time in seconds
 *    d. px: Set expiration time in milliseconds
 *    e. get: get old value while setting new one
 * 
 *    - Input : set like:123 100 nx
 *    - Output: OK
 *    - Input : set like:123 100 nx
 *    - Output: (nil)
 *    - Input : set likes:123 100 xx
 *    - Output: OK
 *    - Input : set likes:123 1M xx
 *    - Output: OK
 *    - Input : set commentsCount 34M px 300 (milliseconds)
 * 
 * Note: There is no fixed order of flags.   
*/

/**
 * Redis Setup:
 * > docker run -d --name redis-stack -p 6379:6370 -p 8001:8001 redis/redis-stack:latest
 * > Copy and keep the Id given in the terminal
 * > Go to http://locahost:8001 (Redis Stack - GUI)
 * > Run: docker ps and copy containerId
 * > Run: docker exec -it <containerId> redis-cli
 *   - Input : PING
 *   - Output: PONG
 * 
 * Client Side Redis Library: ioredis
 * > npm i express ioredis
 * > redis.js to establish redis connection
*/

const express = require('express');
const { redis } = require('./redis.js')
const app = express()

app.get('/', async (req, res) => {
    try {
        const views = await redis.incr('pageViews');
        res.send(`This page is visited: ${views} times.`);
    } catch (error) {
        res.status(500).send('Error accessing Redis');
    }
});

async function main() {
    /* Set Data */ 
    await redis.set("username", "suraj");
    await redis.set("username", "karan")

    /* Get Data */ 
    const username = await redis.get("username");
    console.log("Username", username)

    /* Set Data with Expiry Time */ 
    await redis.set("otp", "123234", { ex: 60 })
    console.log("Otp set with ttl")
}


app.listen(3000, () => {
    main();
    console.log('Server is listening on port 3000')
})