...
export function FileObject({ file, level, setSize, posInSet }) {
  const id = useId();
  ...
  return (
    <li
      ...
      role="treeitem">
      <button
        className={["file-item-button", isDirectory && "file-item-button--directory"].filter(Boolean).join(" ")}
        onClick={() => {
      ...
      >
        <span id={id}>{fileName}</span> {isDirectory && <>[{expanded ? "-" : "+"}]</>}
      </button>
      {fileChildren &&
