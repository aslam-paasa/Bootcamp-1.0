/**
 * Config Driven UI:
 * If we look at the data, it look very complex. Why did swiggy send data
 * like this?
 * - Swiggy follows a different coding style called Config Driven UI.
 * - When we open a swiggy app, we can see offers card at the top like:
 *   a. Grab a 50% off on your first order
 *   b. Pizza treat to brighten your day
 *   c. Explore the authentic chinese cuisine!
 * - These offers are available to me only in Bengaluru.
 * 
 * Suppose there is some other offer in Kolkata, Delhi, Mumbai, etc. then
 * these cards will be different for me. Or in some other city, there
 * might be no offers available. How can we build UI like that?
 * - Our website is driven by data configs, means how our UI will look like
 *   is controlled by data configs.
 *   a. Different data configs for different cities
 *   b. Sort Veg and Non-Veg restaurants
 *   c. Sort restaurants based on their rating
 *   d. If no data available, then no UI will be shown
 *   And this data configs comes from backend.
 * - This is the reason why we see different UI in different cities.
 * 
 * - Now-a-days, all the companies are moving towards this approach.
*/
