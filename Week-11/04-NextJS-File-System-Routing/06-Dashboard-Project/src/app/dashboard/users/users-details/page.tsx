const UserDetails = () => {
  const user = {
    ...
    role: "Admin",
    status: "Active",
    bio: "Rick is a mad scientist who loves to travel through time and space.",
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-100 p-8">
        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-xl p-8">
          <div className="flex items-center mb-8">
            <img src={user.profilePicture} alt={user.name} className="w-24 h-24 rounded-full object-cover mr-6" />
            <div>
              <h1 className="text-3xl font-bold text-gray-800">{user.name}</h1>
              <p className="text-sm text-gray-600">{user.email}</p>
              <p className="text-sm text-gray-600">{user.role}</p>
              <p className={`text-sm ${user.status === 'Active' ? 'text-green-600' : 'text-red-600'}`}>Status: {user.status}</p>
            </div>
          </div>
